...
Sip I/O currently implements three data providers: `redis_data_provider`, `files_data_provider` and `redis_data_provider`. The default data provider is the `files_data_provider`.
...
```yaml
# Users exist in Sip I/O to perform administrative actions on the server
- apiVersion: v1beta1
  kind: User
  metadata:
    name: Administrator
...
```yaml
# Peers and Agents can register in Sip I/O location service
- apiVersion: v1beta1
  kind: Agent
  metadata:
    name: John Doe
...
```yaml
# Peers and Agents can register on Sip I/O location service
- apiVersion: v1beta1
  kind: Peer
  metadata:
