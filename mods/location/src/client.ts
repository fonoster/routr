...
export default class Location implements ILocationService {
  ...
  constructor(config: { addr: string }) {
    this.config = config
    this.location = new LOCATION_OBJECT_PROTO
      .Location(config.addr, grpc.credentials.createInsecure())
  }

  public addRoute(request: AddRouteRequest): Promise<void> {
    return container(this, request, this.addRoute.name) as unknown as Promise<void>
  }
  ...
  public removeRoutes(request: RemoveRoutesRequest): Promise<void> {
    return container(this, request, this.removeRoutes.name) as unknown as Promise<void>
  }
}