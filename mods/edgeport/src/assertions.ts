...
export const isSecureProto = (proto: string) => proto === 'wss' || proto === 'tls'

// We need to have the spec.securityContext for all secure protocol
export const assertHasSecurityContext = (config: EdgePortConfig) => {
  const hasSecureProto = config.spec.transport.some(t1 => isSecureProto(t1.protocol))
...
export const assertNoDuplicatedProto = (transports: Array<Transport>) => {
  if (transports.some(t1 =>
    transports.filter(t2 => t1.protocol === t2.protocol).length > 1
  )) {
...
export const assertNoDuplicatedPort = (transports: Array<Transport>) => {
  ...
  if (transports.some(t1 =>
    transports.filter(t2 => duplicateCondition(t1, t2)).length > 1
  )) {
