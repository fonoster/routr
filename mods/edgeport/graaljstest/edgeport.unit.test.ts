...
describe("@routr/edgeport", () => {
  ...
  it("gets a Map with the properties of the sip stack", () => {
    ...
    expect(properties.get("javax.sip.AUTOMATIC_DIALOG_SUPPORT")).to.be.equal(
      "OFF"
    )
    expect(
      properties.get("gov.nist.javax.sip.MESSAGE_PROCESSOR_FACTORY")
    ).to.be.equal("gov.nist.javax.sip.stack.NioMessageProcessorFactory")
    expect(
      properties.get("gov.nist.javax.sip.PATCH_SIP_WEBSOCKETS_HEADERS")
    ).to.be.equal("false")
    expect(
      properties.get("gov.nist.javax.sip.CACHE_CLIENT_CONNECTIONS")
    ).to.be.equal("true")
    expect(properties.get("gov.nist.javax.sip.REENTRANT_LISTENER")).to.be.equal(
      "false"
    )
    expect(properties.get("gov.nist.javax.sip.THREAD_POOL_SIZE")).to.be.equal(
      "16"
    )
    expect(properties.get("gov.nist.javax.sip.NIO_BLOCKING_MODE")).to.be.equal(
      "NONBLOCKING"
    )
    expect(properties.get("gov.nist.javax.sip.MAX_MESSAGE_SIZE")).to.be.equal(
      "1048576"
    )
    expect(
      properties.get("gov.nist.javax.sip.LOG_MESSAGE_CONTENT")
    ).to.be.equal("false")
    expect(properties.get("javax.sip.IP_ADDRESS")).to.be.equal(
      edgePortConfig.spec.bindAddr
    )
    expect(
      properties.get("gov.nist.javax.sip.TLS_CLIENT_PROTOCOLS")
    ).to.be.equal(edgePortConfig.spec.securityContext.client.protocols.join())
    expect(
      properties.get("gov.nist.javax.sip.TLS_CLIENT_AUTH_TYPE")
    ).to.be.equal(edgePortConfig.spec.securityContext.client.authType)
    expect(properties.get("javax.net.ssl.keyStore")).to.be.equal(
      edgePortConfig.spec.securityContext.keyStore
    )
    expect(properties.get("javax.net.ssl.trustStore")).to.be.equal(
      edgePortConfig.spec.securityContext.trustStore
    )
    expect(properties.get("javax.net.ssl.keyStorePassword")).to.be.equal(
      edgePortConfig.spec.securityContext.keyStorePassword
    )
    expect(properties.get("javax.net.ssl.trustStorePassword")).to.be.equal(
      edgePortConfig.spec.securityContext.trustStorePassword
    )
    expect(properties.get("javax.net.ssl.keyStoreType")).to.be.equal(
      edgePortConfig.spec.securityContext.keyStoreType
    )
  })
  ...
  it("creates a sipStack object", () => {
    ...
    expect(sipStack.getClass().getName()).to.be.equal(
  ...
  describe("assertions", () => {
    it("fails because of missing .spec.securityContext", () => {
      expect(() =>
        assertHasSecurityContext(noSecurityContextEdgePortConfig)
      ).to.throw(
    ...
    it("fails because found duplicated protocol", () => {
      expect(() =>
        assertNoDuplicatedProto(duplicatedProtoEdgePortConfig.spec.transport)
      ).to.throw("found duplicated entries at .spec.transport")
    })

    it("fails because found duplicated port", () => {
      expect(() =>
        assertNoDuplicatedPort(duplicatedPortEdgePortConfig.spec.transport)
      ).to.throw(
  ...
  describe("createListeningPoint", () => {
    it("creates listening points for the given transports", () => {
      const createListeningPointStub = sandbox
        .stub(sipStack, "createListeningPoint")
        .returns(null)
