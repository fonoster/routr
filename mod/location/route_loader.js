  ...
  getDomainEgressRoutes (domainsAPI, numbersAPI, gatewaysAPI) {
    ...
    domains.forEach(domain => {
      if (!isEmpty(domain.spec.context.egressPolicy)) {
        ...
        let response = numbersAPI.getNumber(
          domain.spec.context.egressPolicy.numberRef
        )
        ...
        if (response.status === Status.OK) {
          ...
          if (response.status === Status.OK) {
            ...
            const addressOfRecord = `sip:${
              domain.spec.context.egressPolicy.rule
            }@${domain.spec.context.domainUri}`
            const route = LocatorUtils.buildEgressRoute(
              addressOfRecord,
              gateway,
              number,
              domain
