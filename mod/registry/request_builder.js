 ...
 */
module.exports = (gateway, contactAddr, viaAddr, callId, cseq, userAgent, buildAddr, exp) => {
    ...
    const contactAddress = addressFactory.createAddress(`sip:${contactAddr.host}:${contactAddr.port};bnc`)
    const viaHeader = headerFactory.createViaHeader(viaAddr.host, viaAddr.port, transport, null)
    viaHeader.setRPort()
    ...
    headers.push(headerFactory.createFromHeader(fromAddress, new SipUtils().generateTag()))
    ...
    const request = messageFactory.createRequest(`REGISTER sip:${gwHost} SIP/2.0\r\n\r\n`)
