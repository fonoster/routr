...
describe('Redis Data Source', () => {
  ...
  it('Get collections', function (done) {
    ...
    let response = ds
      ...
      .find('@.spec.credentials.username==1001')
    assert.equal(Status.OK, response.status)

    // Non-Existing Agent
    response = ds
      .withCollection('agents')
      .find("@.spec.credentials.username=='peter'")
    assert.equal(0, response.data.length)

    // Invalid filter
    response = ds
      .withCollection('agents')
      .find("@.spec.credentials.username==mike'")
  ...
  it('Get agents', function (done) {
    ...
    const response = agentsApi.getAgents(
      "@.spec.credentials.username=='1001' || @.spec.credentials.username=='1002'"
    )
